...
proc getNextOp(ops: seq[Operation], index: var int, 
               op: var Operation, amount: var int): bool =
  if amount > 0:
    return true
...
while true:
  ...
  if op1Amount == 0:
    if not getNextOp(ops1, i1, op1Current, op1Amount):
      echo "  No more ops in op1"
      if not getNextOp(ops2, i2, op2Current, op2Amount):
